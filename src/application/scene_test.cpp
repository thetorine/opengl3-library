...
#include "application/scene_test.hpp"
...
namespace gl::app {
    ...
    void SceneTest::initialize() {
        input::registerKeyHandler(std::bind(&SceneTest::keyHandler, this,
                                            std::placeholders::_1,
                                            std::placeholders::_2));
        ...
        m_lightSource = std::make_unique<geometry::Point>(lightPos);
        m_lighting.addPointLight(std::make_shared<illumination::PointLight>(
            lightPos,
            glm::vec3(1.0f),
            1.0f)
        );
        ...
        auto mesh { std::make_shared<geometry::Sphere>(0.5f) };
        m_parentObj = scene::SceneShape::create(mesh);
        m_child1Obj = scene::SceneShape::create(mesh);
        m_child2Obj = scene::SceneShape::create(mesh);
        m_grandchild1Obj = scene::SceneShape::create(mesh);

        m_reparentObj = scene::SceneShape::create(mesh);
